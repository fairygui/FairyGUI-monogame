	...
	public class GPath
	{
		...
		public void GetPointsInSegment(int segmentIndex, float t0, float t1, List<Vector3> points, List<float> ts = null, float pointDensity = 0.1f)
		{
			...
			else if (seg.type == GPathPoint.CurveType.Bezier || seg.type == GPathPoint.CurveType.CubicBezier)
			{
				...
				int SmoothAmount = (int)Math.Min(seg.length * pointDensity, 50);
		...
		{
			int adjustedIndex = (int)Math.Floor(t * (ptCount - 4)) + ptStart; //Since the equation works with 4 points, we adjust the starting point depending on t to return a point on the specific segment
			...
			float adjustedT = (t == 1f) ? 1f : Repeat(t * (ptCount - 4), 1f); // Then we adjust t to be that value on that new piece of segment... for t == 1f don't use repeat (that would return 0f);
		...
		{
			return MathHelper.Clamp(t - (float)Math.Floor(t / length) * length, 0.0f, length);
		}
	}
}