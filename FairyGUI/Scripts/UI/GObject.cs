...
namespace FairyGUI
{
	public class GObject : EventDispatcher
	{
		...
		public void SetPosition(float xv, float yv, float zv)
		{
			if (_x != xv || _y != yv || _z != zv)
			{
				...
				if (draggingObject == this && !sUpdateInDragging)
					sGlobalRect = this.LocalToGlobal(new Rectangle(0, 0, this.width, this.height));
			}
		}
		...
		public Rectangle LocalToGlobal(Rectangle rect)
		{
			...
			Vector2 v = this.LocalToGlobal(new Vector2(rect.X, rect.Y));
			ret.X = v.X;
			ret.Y = v.Y;
			v = this.LocalToGlobal(new Vector2(rect.X + rect.Width, rect.Y + rect.Height));
			ret.Width = v.X - ret.X;
			ret.Height = v.Y - ret.Y;
			return ret;
		}
		...
		{
			Rectangle ret = new Rectangle();
			Vector2 v = this.GlobalToLocal(new Vector2(rect.X, rect.Y));
			ret.X = v.X;
			ret.Y = v.Y;
			v = this.GlobalToLocal(new Vector2(rect.X + rect.Width, rect.Y + rect.Height));
			ret.Width = v.X - ret.X;
			ret.Height = v.Y - ret.Y;
			return ret;
		}
		...
		/// <param name="targetSpace"></param>
		/// <returns></returns>
		public Rectangle TransformRect(Rectangle rect, GObject targetSpace)
		{
			if (_pivotAsAnchor)
			{
		...
		static Rectangle sGlobalRect = new Rectangle();
		...
		private void __touchMove(EventContext context)
		{
			...
			if (_dragTesting && draggingObject != this)
			{
				...
				if (Math.Abs(_dragTouchStartPos.X - evt.x) < sensitivity
					&& Math.Abs(_dragTouchStartPos.Y - evt.y) < sensitivity)
					return;
			...
			if (draggingObject == this)
			{
				...
				if (dragBounds != null)
				{
					Rectangle rect = GRoot.inst.LocalToGlobal((Rectangle)dragBounds);
					if (xx < rect.X)
						xx = rect.X;
					else if (xx + sGlobalRect.Width > rect.X + rect.Width)
					{
						xx = rect.X + rect.Width - sGlobalRect.Width;
						if (xx < rect.X)
							xx = rect.X;
					}
					...
					if (yy < rect.Y)
						yy = rect.Y;
					else if (yy + sGlobalRect.Height > rect.Y + rect.Height)
					{
						yy = rect.Y + rect.Height - sGlobalRect.Height;
						if (yy < rect.Y)
							yy = rect.Y;
					}
				}
				...
				this.SetPosition((float)Math.Round(pt.X), (float)Math.Round(pt.Y));
