	...
	public class GList : GComponent
	{
		...
		public void ScrollToView(int index, bool ani, bool setFirst)
		{
			if (_virtual)
			{
				...
				if (_loop)
					index = (int)Math.Floor((float)_firstIndex / _numItems) * _numItems + index;
		...
		void SetVirtual(bool loop)
		{
			...
			{
				if (this.scrollPane == null)
					Log.Error("FairyGUI: Virtual list must be scrollable!");

				if (loop)
				{
					if (_layout == ListLayoutType.FlowHorizontal || _layout == ListLayoutType.FlowVertical)
						Log.Error("FairyGUI: Loop list is not supported for FlowHorizontal or FlowVertical layout!");
				...
				if (_itemSize.X == 0 || _itemSize.Y == 0)
				{
					...
					if (obj == null)
					{
						Log.Error("FairyGUI: Virtual List must have a default list item resource.");
						_itemSize = new Vector2(100, 100);
					}
					else
					{
						_itemSize = obj.size;
						_itemSize.X = (int)Math.Ceiling(_itemSize.X);
						_itemSize.Y = (int)Math.Ceiling(_itemSize.Y);
						ReturnToPool(obj);
					}
				}
		...
		void RefreshVirtualList(object param)
		{
			...
			if (_realNumItems > 0)
			{
				int len = (int)Math.Ceiling((float)_realNumItems / _curLineItemCount) * _curLineItemCount;
		...
		int GetIndexOnPos3(ref float pos, bool forceUpdate)
		{
			...
			int page = (int)Math.Floor(pos / viewWidth);
		...
		void HandleScroll(bool forceUpdate)
		{
			...
			if (_layout == ListLayoutType.SingleColumn || _layout == ListLayoutType.FlowHorizontal)
			{
				...
				while (HandleScroll1(forceUpdate))
				{
					...
					if (enterCounter > 20)
					{
						Log.Info("FairyGUI: list will never be filled as the item renderer function always returns a different size.");
						break;
					}
				}
		...
		bool HandleScroll1(bool forceUpdate)
		{
			...
			while (curIndex < _realNumItems && (end || curY < max))
			{
				...
				if (needRender)
				{
					...
					{
						deltaSize += (int)Math.Ceiling(ii.obj.size.Y) - ii.size.Y;
						if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex)
						{
							//当内容向下滚动时，如果新出现的项目大小发生变化，需要做一个位置补偿，才不会导致滚动跳动
							firstItemDeltaSize = (int)Math.Ceiling(ii.obj.size.Y) - ii.size.Y;
						}
					}
					ii.size.X = (int)Math.Ceiling(ii.obj.size.X);
					ii.size.Y = (int)Math.Ceiling(ii.obj.size.Y);
				}
		...
		bool HandleScroll2(bool forceUpdate)
		{
			...
			while (curIndex < _realNumItems && (end || curX < max))
			{
				...
				if (needRender)
				{
					...
					if (curIndex % _curLineItemCount == 0)
					{
						deltaSize += (int)Math.Ceiling(ii.obj.size.X) - ii.size.X;
						if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex)
						{
							//当内容向下滚动时，如果新出现的一个项目大小发生变化，需要做一个位置补偿，才不会导致滚动跳动
							firstItemDeltaSize = (int)Math.Ceiling(ii.obj.size.X) - ii.size.X;
						}
					}
					ii.size.X = (int)Math.Ceiling(ii.obj.size.X);
					ii.size.Y = (int)Math.Ceiling(ii.obj.size.Y);
		...
		private void HandleAlign(float contentWidth, float contentHeight)
		{
			...
			if (newOffset != _alignOffset)
			{
				...
				if (scrollPane != null)
					...
					container.SetPosition(_margin.left + _alignOffset.X, _margin.top + _alignOffset.Y, 0);
			}
		}
		...
		override protected void UpdateBounds()
		{
			...
			if (_layout == ListLayoutType.SingleColumn)
			{
				for (i = 0; i < cnt; i++)
				{
					...
					curY += (int)Math.Ceiling(child.height);
					if (child.width > maxWidth)
						maxWidth = child.width;
				}
				cw = (int)Math.Ceiling(maxWidth);
				ch = curY;
			}
			else if (_layout == ListLayoutType.SingleRow)
			{
				for (i = 0; i < cnt; i++)
				{
					...
					curX += (int)Math.Ceiling(child.width);
					if (child.height > maxHeight)
						maxHeight = child.height;
				}
				cw = curX;
				ch = (int)Math.Ceiling(maxHeight);
			}
			else if (_layout == ListLayoutType.FlowHorizontal)
			{
				if (_autoResizeItem && _columnCount > 0)
				{
                    ...
                    for (i = 0; i < cnt; i++)
					{
						...
						if (j == _columnCount || i == cnt - 1)
						{
							...
							//new line
							curY += (int)Math.Ceiling(maxHeight) + _lineGap;
							maxHeight = 0;
							j = 0;
							lineStart = i + 1;
							lineSize = 0;
					...
					ch = curY + (int)Math.Ceiling(maxHeight);
					cw = viewWidth;
				}
				else
				{
				...
				if (_autoResizeItem && _lineCount > 0)
				{
                    ...
                    for (i = 0; i < cnt; i++)
					{
						...
						if (j == _lineCount || i == cnt - 1)
						{
							...
							//new line
							curX += (int)Math.Ceiling(maxWidth) + _columnGap;
							maxWidth = 0;
							j = 0;
							lineStart = i + 1;
							lineSize = 0;
					...
					cw = curX + (int)Math.Ceiling(maxWidth);
					ch = viewHeight;
				}
				else
				{
			...
			else //pagination
			{
				...
				ch = page > 0 ? viewHeight : (curY + (int)Math.Ceiling(maxHeight));
				cw = (page + 1) * viewWidth;
			}
