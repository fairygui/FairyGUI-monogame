...
namespace FairyGUI
{
	...
	public class ScrollPane : EventDispatcher
	{
		...
		public void Setup(ByteBuffer buffer)
		{
				...
				if (_scrollType == ScrollType.Both || _scrollType == ScrollType.Vertical)
				{
					...
					if (!string.IsNullOrEmpty(res))
					{
						...
						if (_vtScrollBar == null)
							Log.Warning("FairyGUI: cannot create scrollbar from " + res);
						else
						{
			...
			if (headerRes != null)
			{
				...
				if (_header == null)
					Log.Warning("FairyGUI: cannot create scrollPane header from " + headerRes);
			}
			...
			if (footerRes != null)
			{
				...
				if (_footer == null)
					Log.Warning("FairyGUI: cannot create scrollPane footer from " + footerRes);
			}
			...
			if (!_maskDisabled && (_vtScrollBar != null || _hzScrollBar != null))
			{
				...
				_owner.rootContainer.clipRect = new Rectangle(0, 0, _owner.width, _owner.height);
			}
		...
		public void SetPercX(float value, bool ani)
		{
			...
			SetPosX(_overlapSize.X * MathHelper.Clamp(value, 0, 1), ani);
		}
		...
		{
			_owner.EnsureBoundsCorrect();
			SetPosY(_overlapSize.Y * MathHelper.Clamp(value, 0, 1), ani);
		}
		...
		public void SetPosX(float value, bool ani)
		{
			...
			value = MathHelper.Clamp(value, 0, _overlapSize.X);
			if (value != _xPos)
			{
				_xPos = value;
				PosChanged(ani);
		...
		public void SetPosY(float value, bool ani)
		{
			...
			value = MathHelper.Clamp(value, 0, _overlapSize.Y);
			if (value != _yPos)
			{
				_yPos = value;
				PosChanged(ani);
		...
		public int currentPageY
		{
			get
			{
				...
				int page = (int)Math.Floor(_yPos / _pageSize.Y);
		...
		public float scrollingPosX
		{
			get
			{
				return MathHelper.Clamp(-_container.x, 0, _overlapSize.X);
			}
		}
		...
		public float scrollingPosY
		{
			get
			{
				return MathHelper.Clamp(-_container.y, 0, _overlapSize.Y);
			}
		}
		...
		/// <param name="ani">If moving to target position with animation</param>
		/// <param name="setFirst">If true, scroll to make the target on the top/left; If false, scroll to make the target any position in view.</param>
		public void ScrollToView(Rectangle rect, bool ani, bool setFirst)
		{
			_owner.EnsureBoundsCorrect();
			if (_needRefresh)
				Refresh();
			...
			if (_overlapSize.Y > 0)
			{
				...
				{
					if (_pageMode)
						this.SetPosY((float)Math.Floor(rect.Y / _pageSize.Y) * _pageSize.Y, ani);
					else
						SetPosY(rect.Y, ani);
				}
			...
			if (_overlapSize.X > 0)
			{
				...
				{
					if (_pageMode)
						this.SetPosX((float)Math.Floor(rect.X / _pageSize.X) * _pageSize.X, ani);
					SetPosX(rect.X, ani);
				}
		...
		public void LockHeader(int size)
		{
			...
			if (!isDispatching("onPullDownRelease") && _container.xy.Get(_refreshBarAxis) >= 0)
			{
				...
				_tweenChange = _tweenChange.Update(_refreshBarAxis, _headerLockedSize - _tweenStart.Get(_refreshBarAxis));
		...
		public void LockFooter(int size)
		{
			...
			if (!isDispatching("onPullUpRelease") && _container.xy.Get(_refreshBarAxis) <= -_overlapSize.Get(_refreshBarAxis))
			{
				_tweenStart = _container.xy;
				_tweenChange = Vector2.Zero;
				float max = _overlapSize.Get(_refreshBarAxis);
				if (max == 0)
					max = Math.Max(_contentSize.Get(_refreshBarAxis) + _footerLockedSize - _viewSize.Get(_refreshBarAxis), 0);
				else
					max += _footerLockedSize;
				_tweenChange = _tweenChange.Update(_refreshBarAxis, -max - _tweenStart.Get(_refreshBarAxis));
				_tweenDuration = new Vector2(TWEEN_TIME_DEFAULT, TWEEN_TIME_DEFAULT);
				_tweenTime = Vector2.Zero;
				_tweening = 2;
				Timers.inst.AddUpdate(_tweenUpdateDelegate);
		...
		internal void AdjustMaskContainer()
		{
			...
			if (_displayOnLeft && _vtScrollBar != null)
				mx = (int)Math.Floor(_owner.margin.left + _vtScrollBar.width);
			else
		...
		{
			if (ToolSet.Approximately(_contentSize.X, aWidth) && ToolSet.Approximately(_contentSize.Y, aHeight))
				return;
		...
		void HandleSizeChanged()
		{
			...
			if (!_maskDisabled)
				_maskContainer.clipRect = new Rectangle(-_owner._alignOffset.X, -_owner._alignOffset.Y, _viewSize.X, _viewSize.Y);

			if (_scrollType == ScrollType.Horizontal || _scrollType == ScrollType.Both)
				_overlapSize.X = (int)Math.Ceiling(Math.Max(0, _contentSize.X - _viewSize.X));
			else
				_overlapSize.X = 0;
			if (_scrollType == ScrollType.Vertical || _scrollType == ScrollType.Both)
				_overlapSize.Y = (int)Math.Ceiling(Math.Max(0, _contentSize.Y - _viewSize.Y));
			else
				_overlapSize.Y = 0;

			//边界检查
			_xPos = MathHelper.Clamp(_xPos, 0, _overlapSize.X);
			_yPos = MathHelper.Clamp(_yPos, 0, _overlapSize.Y);
			float max = _overlapSize.Get(_refreshBarAxis);
			if (max == 0)
				max = Math.Max(_contentSize.Get(_refreshBarAxis) + _footerLockedSize - _viewSize.Get(_refreshBarAxis), 0);
			else
			...
			if (_refreshBarAxis == 0)
				_container.SetPosition(MathHelper.Clamp(_container.x, -max, _headerLockedSize), MathHelper.Clamp(_container.y, -_overlapSize.Y, 0));
			else
				_container.SetPosition(MathHelper.Clamp(_container.x, -_overlapSize.X, 0), MathHelper.Clamp(_container.y, -max, _headerLockedSize));
		...
		private void SyncScrollBar(bool end = false)
		{
			if (_vtScrollBar != null)
			{
				_vtScrollBar.scrollPerc = _overlapSize.Y == 0 ? 0 : MathHelper.Clamp(-_container.y, 0, _overlapSize.Y) / _overlapSize.Y;
				if (_scrollBarDisplayAuto)
					ShowScrollBar(!end);
			}
			if (_hzScrollBar != null)
			{
				_hzScrollBar.scrollPerc = _overlapSize.X == 0 ? 0 : MathHelper.Clamp(-_container.x, 0, _overlapSize.X) / _overlapSize.X;
				if (_scrollBarDisplayAuto)
					ShowScrollBar(!end);
			}
		...
		private void __touchMove(EventContext context)
		{
			...
			if (_scrollType == ScrollType.Vertical)
			{
				if (!_isHoldAreaDone)
				{
					...
					{
						float diff2 = Math.Abs(_beginTouchPos.X - pt.X);
			...
			else if (_scrollType == ScrollType.Horizontal)
			{
				if (!_isHoldAreaDone)
				{
					...
					if ((_gestureFlag & 1) != 0)
					{
						float diff2 = Math.Abs(_beginTouchPos.Y - pt.Y);
						if (diff < diff2)
							return;
					}
				}
			...
			if (sv)
			{
				if (newPos.Y > 0)
				{
					if (!_bouncebackEffect)
						...
						_container.y = (int)Math.Min(newPos.Y * 0.5f, _header.maxHeight);
					else
						_container.y = (int)Math.Min(newPos.Y * 0.5f, _viewSize.Y * PULL_RATIO);
				}
				else if (newPos.Y < -_overlapSize.Y)
				{
					if (!_bouncebackEffect)
						...
						_container.y = (int)Math.Max((newPos.Y + _overlapSize.Y) * 0.5f, -_footer.maxHeight) - _overlapSize.Y;
					else
						_container.y = (int)Math.Max((newPos.Y + _overlapSize.Y) * 0.5f, -_viewSize.Y * PULL_RATIO) - _overlapSize.Y;
				}
				else
			...
			if (sh)
			{
				if (newPos.X > 0)
				{
					if (!_bouncebackEffect)
						...
						_container.x = (int)Math.Min(newPos.X * 0.5f, _header.maxWidth);
					else
						_container.x = (int)Math.Min(newPos.X * 0.5f, _viewSize.X * PULL_RATIO);
				}
				else if (newPos.X < 0 - _overlapSize.X)
				{
					if (!_bouncebackEffect)
						...
						_container.x = (int)Math.Max((newPos.X + _overlapSize.X) * 0.5f, -_footer.maxWidth) - _overlapSize.X;
					else
						_container.x = (int)Math.Max((newPos.X + _overlapSize.X) * 0.5f, -_viewSize.X * PULL_RATIO) - _overlapSize.X;
				}
				else
			...
			//更新速度
			float deltaTime = (float)Stage.game.TargetElapsedTime.TotalMilliseconds / 1000f;
			float elapsed = (Timers.time - _lastMoveTime) * 60 - 1;
			if (elapsed > 1) //速度衰减
				_velocity = _velocity * (float)Math.Pow(0.833f, elapsed);
			...
			if (deltaPosition.X != 0)
				_velocityScale = Math.Abs(deltaGlobalPosition.X / deltaPosition.X);
			else if (deltaPosition.Y != 0)
				_velocityScale = Math.Abs(deltaGlobalPosition.Y / deltaPosition.Y);
			...
			//同步更新pos值
			if (_overlapSize.X > 0)
				_xPos = MathHelper.Clamp(-_container.x, 0, _overlapSize.X);
			if (_overlapSize.Y > 0)
				_yPos = MathHelper.Clamp(-_container.y, 0, _overlapSize.Y);
		...
		private void __touchEnd(EventContext context)
		{
			...
			if (flag)
			{
				...
				{
					endPos = endPos.Update(_refreshBarAxis, _headerLockedSize);
					_tweenChange = endPos - _tweenStart;
				}
				else if (_footerLockedSize > 0 && endPos.Get(_refreshBarAxis) == -_overlapSize.Get(_refreshBarAxis))
				{
					float max = _overlapSize.Get(_refreshBarAxis);
					if (max == 0)
						max = Math.Max(_contentSize.Get(_refreshBarAxis) + _footerLockedSize - _viewSize.Get(_refreshBarAxis), 0);
						...
						max += _footerLockedSize;
					endPos = endPos.Update(_refreshBarAxis, -max);
					_tweenChange = endPos - _tweenStart;
				}
		...
		{
			return (_contentSize.Get(axis) + (axis == 0 ? ((GList)_owner).columnGap : ((GList)_owner).lineGap)) / division;
		}
		...
		{
			if (endPos.Get(axis) > 0)
			{
				float halfSize = GetLoopPartSize(2, axis);
				float tmp = _tweenStart.Get(axis) - halfSize;
				if (tmp <= 0 && tmp >= -_overlapSize.Get(axis))
				{
					endPos = endPos.Update(axis, endPos.Get(axis) - halfSize);
					_tweenStart = _tweenStart.Update(axis, tmp);
				}
			}
			else if (endPos.Get(axis) < -_overlapSize.Get(axis))
			{
				float halfSize = GetLoopPartSize(2, axis);
				float tmp = _tweenStart.Get(axis) + halfSize;
				if (tmp <= 0 && tmp >= -_overlapSize.Get(axis))
		...
		void LoopCheckingNewPos(ref float value, int axis)
		{
			...
			if (value < 0.001f)
			{
				...
				if (value > pos)
				{
					...
					v = (int)Math.Ceiling((value - pos) / v) * v;
					pos = MathHelper.Clamp(pos + v, 0, _overlapSize.Get(axis));
					changed = true;
				}
			}
			else if (value >= _overlapSize.Get(axis))
			{
				value -= GetLoopPartSize(2, axis);
				if (value < pos)
				{
					float v = GetLoopPartSize(6, axis);
					v = (int)Math.Ceiling((pos - value) / v) * v;
					pos = MathHelper.Clamp(pos - v, 0, _overlapSize.Get(axis));
					changed = true;
				}
			}
		...
		float AlignByPage(float pos, int axis, bool inertialScrolling)
		{
			...
			if (pos > 0)
				page = 0;
			else if (pos < -_overlapSize.Get(axis))
				page = (int)Math.Ceiling(_contentSize.Get(axis) / _pageSize.Get(axis)) - 1;
			else
			{
				page = (int)Math.Floor(-pos / _pageSize.Get(axis));
				float change = inertialScrolling ? (pos - _containerPos.Get(axis)) : (pos - _container.xy.Get(axis));
				float testPageSize = Math.Min(_pageSize.Get(axis), _contentSize.Get(axis) - (page + 1) * _pageSize.Get(axis));
				float delta = -pos - page * _pageSize.Get(axis);

				//页面吸附策略
				if (Math.Abs(change) > _pageSize.Get(axis))//如果滚动距离超过1页,则需要超过页面的一半，才能到更下一页
				{
				...
				//重新计算终点
				pos = -page * _pageSize.Get(axis);
				if (pos < -_overlapSize.Get(axis)) //最后一页未必有pageSize那么大
					pos = -_overlapSize.Get(axis);
			}

			//惯性滚动模式下，会增加判断尽量不要滚动超过一页
			if (inertialScrolling)
			{
				float oldPos = _tweenStart.Get(axis);
				int oldPage;
				if (oldPos > 0)
					oldPage = 0;
				else if (oldPos < -_overlapSize.Get(axis))
					oldPage = (int)Math.Ceiling(_contentSize.Get(axis) / _pageSize.Get(axis)) - 1;
				else
					oldPage = (int)Math.Floor(-oldPos / _pageSize.Get(axis));
				int startPage = (int)Math.Floor(-_containerPos.Get(axis) / _pageSize.Get(axis));
		...
		float UpdateTargetAndDuration(float pos, int axis)
		{
			...
			{
				//以屏幕像素为基准
				float v2 = Math.Abs(v) * _velocityScale;
				//在移动设备上，需要对不同分辨率做一个适配，我们的速度判断以1136分辨率为基准
				if (Stage.touchScreen)
					v2 *= 1136f / Math.Max(Stage.inst.width, Stage.inst.height);
				//这里有一些阈值的处理，因为在低速内，不希望产生较大的滚动（甚至不滚动）
				float ratio = 0;
				if (_pageMode || !Stage.touchScreen)
				{
					if (v2 > 500)
						ratio = (float)Math.Pow((v2 - 500) / 500, 2);
				}
				else
				{
					if (v2 > 1000)
						ratio = (float)Math.Pow((v2 - 1000) / 1000, 2);
				}
				...
				if (ratio != 0)
				{
					...
					v2 *= ratio;
					v *= ratio;
					_velocity = _velocity.Update(axis, v);

					//算法：v*（_decelerationRate的n次幂）= 60，即在n帧后速度降为60（假设每秒60帧）。
					duration = (float)Math.Log(60 / v2, _decelerationRate) / 60;
			...
			_tweenDuration = _tweenDuration.Update(axis, duration);
		...
		{
			if (_tweenChange.Get(axis) == 0 || Math.Abs(_tweenChange.Get(axis)) >= Math.Abs(oldChange))
				return;

			float newDuration = Math.Abs(_tweenChange.Get(axis) / oldChange) * _tweenDuration.Get(axis);
			if (newDuration < TWEEN_TIME_DEFAULT)
				newDuration = TWEEN_TIME_DEFAULT;

			_tweenDuration = _tweenDuration.Update(axis, newDuration);
		}
		...
		void CheckRefreshBar()
		{
			...
			float pos = _container.xy.Get(_refreshBarAxis);
			if (_header != null)
			{
				if (pos > 0)
				{
					...
					vec = _header.size;
					vec = vec.Update(_refreshBarAxis, pos);
					_header.size = vec;
				}
				else
				{
			...
			if (_footer != null)
			{
				...
				if (pos < -max || max == 0 && _footerLockedSize > 0)
				{
					...
					vec = _footer.xy;
					if (max > 0)
						vec = vec.Update(_refreshBarAxis, pos + _contentSize.Get(_refreshBarAxis));
					else
						vec = vec.Update(_refreshBarAxis, Math.Max(Math.Min(pos + _viewSize.Get(_refreshBarAxis), _viewSize.Get(_refreshBarAxis) - _footerLockedSize), _viewSize.Get(_refreshBarAxis) - _contentSize.Get(_refreshBarAxis)));
					_footer.xy = vec;

					vec = _footer.size;
					if (max > 0)
						vec = vec.Update(_refreshBarAxis, -max - pos);
					else
						vec = vec.Update(_refreshBarAxis, _viewSize.Get(_refreshBarAxis) - _footer.xy.Get(_refreshBarAxis));
					_footer.size = vec;
				}
				else
				{
		...
		void TweenUpdate(object param)
		{
			...
			{
				if (_overlapSize.X > 0)
					_xPos = MathHelper.Clamp(-nx, 0, _overlapSize.X);
				if (_overlapSize.Y > 0)
					_yPos = MathHelper.Clamp(-ny, 0, _overlapSize.Y);
		...
		{
			float newValue;
			if (_tweenChange.Get(axis) != 0)
			{
				_tweenTime = _tweenTime.Update(axis, _tweenTime.Get(axis) + Timers.deltaTime);
				if (_tweenTime.Get(axis) >= _tweenDuration.Get(axis))
				{
					newValue = _tweenStart.Get(axis) + _tweenChange.Get(axis);
					_tweenChange = _tweenChange.Update(axis, 0);
				}
				else
				{
					float ratio = EaseFunc(_tweenTime.Get(axis), _tweenDuration.Get(axis));
					newValue = _tweenStart.Get(axis) + (int)(_tweenChange.Get(axis) * ratio);
				}

				float threshold1 = 0;
				float threshold2 = -_overlapSize.Get(axis);
				...
				if (_footerLockedSize > 0 && _refreshBarAxis == axis)
				{
					...
					if (max == 0)
						max = Math.Max(
							_contentSize.Get(_refreshBarAxis) + _footerLockedSize - _viewSize.Get(_refreshBarAxis), 0);
					else
						max += _footerLockedSize;
					threshold2 = -max;
				}
				...
				{
					if (newValue > 20 + threshold1 && _tweenChange.Get(axis) > 0
						|| newValue > threshold1 && _tweenChange.Get(axis) == 0) //开始回弹
					{
						_tweenTime = _tweenTime.Update(axis, 0);
						_tweenDuration = _tweenDuration.Update(axis, TWEEN_TIME_DEFAULT);
						_tweenChange = _tweenChange.Update(axis, -newValue + threshold1);
						_tweenStart = _tweenStart.Update(axis, newValue);
					}
					else if (newValue < threshold2 - 20 && _tweenChange.Get(axis) < 0
							 || newValue < threshold2 && _tweenChange.Get(axis) == 0) //开始回弹
					{
						_tweenTime.Update(axis, 0);
						_tweenDuration = _tweenDuration.Update(axis, TWEEN_TIME_DEFAULT);
						_tweenChange = _tweenChange.Update(axis, threshold2 - newValue);
						_tweenStart = _tweenStart.Update(axis, newValue);
					}
				}
				else
				...
				newValue = _container.xy.Get(axis);
	...
	{
		public static float Get(this Vector2 vector2, int index)
		{
			switch (index)
			{
				case 0:
		...
		public static Vector2 Update(this Vector2 vector2, int index, float value)
		{
			switch (index)
			{
				case 0:
