	...
	public class Transition : ITweenListener
	{
		...
		void StopItem(TransitionItem item, bool setToComplete)
		{
			...
			if (item.tweener != null)
			{
				...
				if (item.type == TransitionActionType.Shake && !setToComplete) //震动必须归位，否则下次就越震越远了。
				{
					...
					item.target.SetPosition(item.target.x - ((TValue_Shake)item.value).lastOffset.X, item.target.y - ((TValue_Shake)item.value).lastOffset.Y);
		...
		public void SetDuration(string label, float value)
		{
			...
			for (int i = 0; i < cnt; i++)
			{
				...
				{
					item.tweenConfig.duration = value; item.tweenConfig.duration = value;
					found = true;
				}
			}
		...
		void ApplyValue(TransitionItem item)
		{
			...
			switch (item.type)
			{
				...
				case TransitionActionType.Shake:
					{
						TValue_Shake value = (TValue_Shake)item.value;
						item.target.SetPosition(item.target.x - value.lastOffset.X + value.offset.X, item.target.y - value.lastOffset.Y + value.offset.Y);
				...
				case TransitionActionType.Sound:
					if (_playing && item.time >= _startTime)
					{
						...
						if (value.audioClip == null)
							value.audioClip = UIPackage.GetItemAssetByURL(value.sound) as SoundEffectInstance;
