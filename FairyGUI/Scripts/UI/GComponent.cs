	...
	public class GComponent : GObject
	{
		...
		protected void SetupOverflow(OverflowType overflow)
		{
			if (overflow == OverflowType.Hidden)
			{
				...
				UpdateClipRect();
				container.SetPosition(_margin.left, _margin.top, 0);
			}
		...
		void UpdateClipRect()
		{
			if (scrollPane == null)
			{
				...
				rootContainer.clipRect = new Rectangle(_margin.left, _margin.top, w, h);
			}
			else
				rootContainer.clipRect = new Rectangle(0, 0, this.width, this.height);
		}
		...
		protected void SetBounds(float ax, float ay, float aw, float ah)
		{
			...
			if (scrollPane != null)
				scrollPane.SetContentSize((float)Math.Round(ax + aw), (float)Math.Round(ay + ah));
		}
